...
const autoSeed = async () => {
  ...
  if (count === 0) {
    ...
    const village = await Village.create({ name: 'Rampur', code: 'VIL001', district: 'Guntur', state: 'Andhra Pradesh', population: 2500, totalHouseholds: 450, waterSources: ['borewell', 'hand_pump', 'river'], location: { type: 'Point', coordinates: [80.4365, 16.3067] }, gpAdmin: admin._id, vWSC: [admin._id], isActive: true });
    console.log('Village seeded');

    const pump1 = await Pump.create({ name: 'Main Borewell Pump', pumpId: 'PMP001', village: village._id, location: { type: 'Point', coordinates: [80.4370, 16.3062] }, type: 'submersible', capacity: 5, runningHours: 3200, powerConsumption: 4.2, efficiencyScore: 85, voltage: 220, temperature: 45, status: 'running', lastMaintenance: new Date('2024-01-15'), nextMaintenance: new Date('2024-04-15'), installedDate: new Date('2022-06-01') });
    const pump2 = await Pump.create({ name: 'School Pump', pumpId: 'PMP002', village: village._id, location: { type: 'Point', coordinates: [80.4358, 16.3072] }, type: 'centrifugal', capacity: 3, runningHours: 1500, powerConsumption: 2.8, efficiencyScore: 92, voltage: 230, temperature: 40, status: 'running', lastMaintenance: new Date('2024-02-10'), nextMaintenance: new Date('2024-05-10'), installedDate: new Date('2023-01-15') });
    const pump3 = await Pump.create({ name: 'Backup Pump', pumpId: 'PMP003', village: village._id, location: { type: 'Point', coordinates: [80.4362, 16.3069] }, type: 'booster', capacity: 2, runningHours: 800, powerConsumption: 1.8, efficiencyScore: 78, voltage: 215, temperature: 42, status: 'maintenance', lastMaintenance: new Date('2024-03-01'), nextMaintenance: new Date('2024-06-01'), installedDate: new Date('2023-06-20') });
    console.log('Pumps seeded');

    const tank1 = await WaterTank.create({ name: 'Overhead Tank - Main', tankId: 'TNK001', village: village._id, location: { type: 'Point', coordinates: [80.4367, 16.3064] }, capacity: 50000, currentLevel: 35000, dailyConsumption: 8000, type: 'overhead', status: 'normal', lastRefilled: new Date('2024-03-10') });
    const tank2 = await WaterTank.create({ name: 'Underground Tank - East', tankId: 'TNK002', village: village._id, location: { type: 'Point', coordinates: [80.4372, 16.3061] }, capacity: 30000, currentLevel: 5000, dailyConsumption: 5000, type: 'underground', status: 'low', lastRefilled: new Date('2024-03-05') });
    console.log('Tanks seeded');

    const pipeline1 = await Pipeline.create({ name: 'Main Distribution Pipeline', pipelineId: 'PIPE001', village: village._id, length: 2500, diameter: 150, material: 'HDPE', status: 'good', installationDate: new Date('2022-06-01'), coordinates: { type: 'LineString', coordinates: [[80.4365, 16.3067],[80.4370, 16.3062],[80.4372, 16.3061],[80.4375, 16.3064]] } });
    const pipeline2 = await Pipeline.create({ name: 'Secondary Pipeline - East', pipelineId: 'PIPE002', village: village._id, length: 1200, diameter: 100, material: 'PVC', status: 'fair', leakReports: [{ reportedBy: citizen._id, reportedAt: new Date('2024-03-08'), description: 'Small leak near junction box', status: 'reported' }], installationDate: new Date('2023-01-15'), coordinates: { type: 'LineString', coordinates: [[80.4372, 16.3061],[80.4376, 16.3058],[80.4380, 16.3055]] } });
    console.log('Pipelines seeded');

    await Valve.create({ name: 'Main Gate Valve', valveId: 'VLV001', village: village._id, pipeline: pipeline1._id, status: 'open', type: 'gate', lastChecked: new Date('2024-03-10'), diameter: 150, location: { type: 'Point', coordinates: [80.4370, 16.3062] } });
    await Valve.create({ name: 'Secondary Gate Valve', valveId: 'VLV002', village: village._id, pipeline: pipeline1._id, status: 'closed', type: 'gate', lastChecked: new Date('2024-03-10'), diameter: 100, location: { type: 'Point', coordinates: [80.4372, 16.3061] } });
    await Valve.create({ name: 'East Pipeline Butterfly Valve', valveId: 'VLV003', village: village._id, pipeline: pipeline2._id, status: 'partially_open', type: 'butterfly', lastChecked: new Date('2024-03-09'), diameter: 100, location: { type: 'Point', coordinates: [80.4376, 16.3058] } });
    await Valve.create({ name: 'Check Valve - Pump Station', valveId: 'VLV004', village: village._id, pipeline: pipeline1._id, status: 'open', type: 'check', lastChecked: new Date('2024-03-10'), diameter: 150, location: { type: 'Point', coordinates: [80.4365, 16.3067] } });
    console.log('Valves seeded');

    const statuses = ['safe', 'safe', 'needs_inspection', 'unsafe', 'safe'];
    const qr = [];
    for (let i = 0; i < 5; i++) {
      qr.push(await WaterQuality.create({ village: village._id, recordedBy: admin._id, location: { type: 'Point', coordinates: [80.4365 + (i * 0.001), 16.3067 + (i * 0.001)] }, parameters: { pH: 7.0 + (Math.random() * 1.5 - 0.5), TDS: 200 + Math.floor(Math.random() * 300), turbidity: Math.random() * 6, chlorine: 0.2 + Math.random() * 0.8, fluoride: Math.random() * 1.5, iron: Math.random() * 0.4, nitrate: Math.random() * 40, coliform: Math.floor(Math.random() * 3) }, overallStatus: statuses[i], recommendations: statuses[i] === 'unsafe' ? ['Boil water before use'] : [], sampleDate: new Date(Date.now() - (i * 86400000)) }));
    }
    ...
    for (let i = 0; i < 10; i++) {
      await Complaint.create({ complainant: citizen._id, village: village._id, category: cats[i % cats.length], description: `Complaint ${i + 1}: Sample ${cats[i % cats.length]} issue.`, location: { type: 'Point', coordinates: [80.4365 + (Math.random() * 0.005), 16.3067 + (Math.random() * 0.005)] }, status: sts[i % sts.length], assignedTo: i % 2 === 0 ? admin._id : undefined, priority: pris[i % pris.length] });
    }
